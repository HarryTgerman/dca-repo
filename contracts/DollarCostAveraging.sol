...
contract DollarCostAveraging {
    ...
    constructor(address _wrappedNative, address _factory, bytes32 _initCodeHash) {
        ...
        internal
    {
        ...
        uint[] amounts = UniswapV2Library.getAmountsOut(FACTORY_ADDRESS, msg.value, path, INIT_CODE_HASH);

        if(amount > (fee + _order.epochAmount)) {
            orderValue[key] = amount - (fee + _order.epochAmount);
            orderUpdatedAt[key] = now;
