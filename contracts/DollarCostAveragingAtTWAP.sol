...
// can only be used to DCA into one specific ERC20 Token, as outputAmount is tied to TWAP
...
contract DollarCostAveragingAtTWAP {
    ...
    function depositOrder(UniswapV2DCAOrder calldata _order) external payable {
        require(
            msg.value != 0,
            "DollarCostAveraging.depositNativeOrder: VALUE_IS_0"
        );
        require(
            _order.expiryDate > block.timestamp,
            "DollarCostAveraging.depositNativeOrder: ORDER_EXPIRED"
        );
        require(
        ...
        if (amount > (_fee + _order.epochAmount)) {
            ...
            require(
                amounts[amounts.length - 1] >=
                    IExampleOracleSimple(ORACLE_ADDRESS).consult(
                        _order.outputToken,
                        DCAAmount
                    ),
                "AMOUNT_OUT_LESS_THAN_TWAP"
            );
